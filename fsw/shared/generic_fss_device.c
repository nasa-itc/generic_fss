...
static int32_t GENERIC_FSS_ReadData(spi_info_t *device, uint8_t *write_data,
                                    uint8_t *read_data, uint8_t data_length);
static int32_t GENERIC_FSS_CommandDevice(spi_info_t *device, uint8_t cmd);
static uint8_t compute_checksum(uint8_t in[], int starting_byte,
                                int number_of_bytes);
...
*/
int32_t GENERIC_FSS_RequestData(spi_info_t *device,
                                GENERIC_FSS_Device_Data_tlm_t *data) {
  ...
  if (status == OS_SUCCESS) {
    status =
        GENERIC_FSS_ReadData(device, write_data, read_data, sizeof(read_data));
    if (status == OS_SUCCESS) {
      ...
      if ((read_data[0] == GENERIC_FSS_DEVICE_HDR_0) &&
          ...
          (read_data[4] == 0x01) && (read_data[5] == 0x0A) &&
        ...
        OS_printf("  Header           = 0x%02x%02x%02x%02x \n", read_data[0],
                  read_data[1], read_data[2], read_data[3]);
...
#ifdef GENERIC_FSS_CFG_DEBUG
    OS_printf("  GENERIC_FSS_RequestData: GENERIC_FSS_CommandDevice reported "
              "error %d \n",
              status);
#endif
  }
...
static int32_t GENERIC_FSS_ReadData(spi_info_t *device, uint8_t *write_data,
  ...
  if ((spi_select_chip(device) == SPI_SUCCESS) &&
      (spi_transaction(device, write_data, read_data, data_length,
                       GENERIC_FSS_CFG_DELAY, GENERIC_FSS_CFG_BITS_PER_WORD,
                       0) == SPI_SUCCESS) &&
...
static int32_t GENERIC_FSS_CommandDevice(spi_info_t *device, uint8_t cmd_code) {
  ...
  /* Write data */
  if ((spi_select_chip(device) == SPI_SUCCESS) &&
      (spi_transaction(device, write_data, read_data,
                       GENERIC_FSS_DEVICE_CMD_SIZE, GENERIC_FSS_CFG_DELAY,
...
static float fourbytes_little_endian_to_float(uint8_t bytes[]) {
  ...
  fu.u = ((uint32_t)bytes[3] << 24) | ((uint32_t)bytes[2] << 16) |
         ((uint32_t)bytes[1] << 8) | (uint32_t)bytes[0];
#elif __BYTE_ORDER__ == __ORDER_BIG_ENDIAN__
  fu.u = ((uint32_t)bytes[0] << 24) | ((uint32_t)bytes[1] << 16) |
